...
if (+process.version.slice(1).split(".")[0] < 16) {
  console.log(
    "\u001b[31mCorebot requires Node JS version 16 or higher. Please go to https://nodejs.org/en/ then download and install the LTS version.\033[0m",
  );
  process.exit();
}
...
const installModules = async () => {
  return new Promise(async (resolve) => {
    if (
      ...
      const showInfo = process.argv
        .slice(2)
        .map((a) => a.toLowerCase())
        .includes("--show-install-output");
      ...
      const missingModules = modules.filter((module) => {
        try {
          ...
          return (
            module !== "n" &&
            !err.toString().includes("Error [ERR_PACKAGE_PATH_NOT_EXPORTED]")
          );
        }
      });
      ...
      if (missingModules.length == 0) {
        ...
        console.log(
          ...
          `module${missingModules.length == 1 ? " is" : "s are"} not installed... Installing...`,
        );
        ...
        if (missingModules.length == 21) {
          ...
          for (let i = 0; i < missingModules.length; i++) {
            ...
            console.log(
              info,
              `Installing module ${i + 1}/${missingModules.length} (${module})`,
            );
            ...
            await new Promise((resolve) => {
              ...
              install.on("exit", () => {
                console.log(
                  info,
                  `Finished installing module ${i + 1}/${missingModules.length} (${(((i + 1) / missingModules.length) * 100).toFixed(2)}% done)`,
                );
                resolve();
              });
            });
        ...
        console.log(
          info,
          "All missing modules have been installed! Please restart the bot",
        );
        process.exit();
      }
    }
...
installModules().then(async () => {
  ...
  // DATABASE
  const Database = await require("./src/modules/database.js").setup(
    config,
    bot,
  );
  ...
  Utils.yml("./configs/config.yml").then((config) => {
    bot.login(process.env.TOKEN).catch((error) => {
      if (error.message.includes("An invalid token was provided")) {
        ...
        console.log(
          Utils.errorPrefix +
            "An error occured while attempting to login to the bot:",
        );
        console.log(error);
      }
      process.exit();
